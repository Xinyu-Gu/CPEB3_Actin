...
simulation.minimizeEnergy(tolerance=Quantity(value=10, unit=kilojoule/mole))  # first, minimize the energy to a local minimum to reduce any large forces that might be present
simulation.step(int(1))

integrator = LangevinIntegrator(600*kelvin, 1/picosecond, 2*femtoseconds)
integrator.setRandomNumberSeed(20436)
simulation = Simulation(oa.pdb.topology, oa.system, integrator, platform)
simulation.context.setPositions(oa.pdb.positions) # set the initial positions of the atoms
...
for i in range(50):
      integrator.setTemperature(3*(150-i)*kelvin)
      simulation.step(int(4e4))
