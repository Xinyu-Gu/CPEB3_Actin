...
#integrator.setRandomNumberSeed(31750)
simulation = Simulation(oa.pdb.topology, oa.system, integrator, platform)
simulation.loadCheckpoint(file="./restart") 
#simulation.context.setPositions(oa.pdb.positions) # set the initial positions of the atoms
...
simulation.reporters.append(PDBReporter(os.path.join(args.to, "movie-1.pdb"), reporter_frequency))  # output PDBs of simulated structures
...
if args.simulation_mode == 0:
    ...
    for i in range(100):
        integrator.setTemperature((300-i)*kelvin)
