...
anti_index = np.loadtxt('anti_go.index', dtype='int')
...
#    rama_ssweight_term(oa),
    ...
    additive_amhgo_term(oa,"7-actins.pdb", k_amhgo=209.2),
    additive_anti_go_term(oa, anti_index, 7), 
#    er_term(oa, k_er=4.184),
...
simulation.minimizeEnergy(tolerance=Quantity(value=3, unit=kilojoule/mole))  # first, minimize the energy to a local minimum to reduce any large forces that might be present
simulation.step(int(1))

integrator = LangevinIntegrator(400*kelvin, 1/picosecond, 2*femtoseconds)
...
if args.simulation_mode == 0:
    ...
    for i in range(100):
        integrator.setTemperature((400-i)*kelvin)
